...
class GlobalRoutePlanner(object):
    ...
    def trace_route(self, origin, destination):
        """
        This method returns list of carla.Waypoint
        ...
        if len(route_trace) > 0:
            cur_waypt = route_trace[0]
            filtered_route.append(cur_waypt)

            for waypt in route_trace[1:]:
                if cur_waypt.transform.location.distance(waypt.transform.location) > 1e-3:
                    filtered_route.append(waypt)
                    cur_waypt = waypt
